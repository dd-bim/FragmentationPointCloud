...
public readonly struct Plane
{
    ...
    public Plane(in Vector a, in Vector b, in Vector c, bool aAsPosition = false)
    {
        ...
        Vector pos = aAsPosition ? a : Vector.Mean(a, b, c);
    ...
    public static bool Create(in IReadOnlyList<Vector> vectors, out Plane plane, bool isRing = false)
    {
        ...
        switch (len)
        {
            ...
            default:
                ...
                for (var i = 0; i < len; i++) red[i] = vectors[i] - mean;
                if (isRing)
                {
                    red[^1] = red[0];
                    Vector vnormal = red[0].Cross(red[1]);
                    for (var i = 2; i < red.Length; i++) vnormal += red[i - 1].Cross(red[i]);
                    plane = new Plane(vectors[0], (Direction)vnormal, (Direction)(vectors[1] - vectors[0]));
                    return true;
                }

                SpdMatrix cov = red[0].Outer().AddCov(red[1]);
                for (var i = 2; i < len; i++) cov = cov.AddCov(red[i]);
                Quaternion quat = Decomposition.EigenVectors(cov);
    ...
    {
        return System.FromPlaneSystem(vector);
    }

    public Direction FromSystem(in D2.Direction direction)
    {
        return System.FromSystem(direction);
    }

    public Vector[] FromSystem(in IReadOnlyList<D2.Vector> vectors)
    {
        return System.FromSystem(vectors);
    }
    ...
    public Plane Turn()
    {
        return new Plane(Position, Normal.Turn(), PlaneX);
        // Achtung evtl. Polygon passt dann nicht mehr!
    }
    ...
    {
        return System.ApproxEquals(other, maxDifferenceD, maxDifferenceCosOne);
    }
