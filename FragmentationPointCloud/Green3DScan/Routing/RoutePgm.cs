...
namespace Revit.Green3DScan.Routing
{
    ...
    {
        private static Result CreateLevel(Document doc, SettingsJson set, out Level newLevel)
        {
            try
            {
                ...
                string baseName = "Route_PGM_High_" + set.PgmHeightOfLevel_Meter;
        ...
        private static Result CreateViewPlan(Document doc, UIDocument uidoc, Level newLevel)
        {
            try
            {
                ViewPlan newView;
                using (var tx = new Transaction(doc, "Create ViewPlan"))
                {
                    tx.Start();
                ...
                using (var tx = new Transaction(doc, "Change ViewDetailLevel"))
                {
                    tx.Start();
                ...
                // get all categories
                var cat = GetBuiltInCategory();
                foreach (BuiltInCategory category in cat)
                {
                    // get all existing elements to the categories
                    var categoryFilter = new ElementCategoryFilter(category);
                    var filteredLabels = new FilteredElementCollector(doc, newView.Id);
                    filteredLabels.WherePasses(categoryFilter);
                    var listElementId = new List<ElementId>();
                    foreach (Element element in filteredLabels) listElementId.Add(element.Id);
                    if (listElementId.Count == 0) continue;

                    // hide elements
                    var tx = new Transaction(doc, "Hide Elements");
                    tx.Start();
                    try
                    {
            ...
            {
                var cat = new List<BuiltInCategory>
                {
        ...
        private static Result RoomColorScheme(Document doc, UIApplication uiapp)
        {
            try
            {
                var roomCategory = Category.GetCategory(doc, BuiltInCategory.OST_Rooms);
                ...
                using (var tx = new Transaction(doc, "Create ColorScheme"))
                {
                    tx.Start();
                    ...
                    var colorScheme = doc.GetElement(newColorSchemeId) as ColorFillScheme;
                    schemeId = colorScheme.Id;
                    tx.Commit();
                }

                var colorSchemeChange = doc.GetElement(schemeId) as ColorFillScheme;

                using (var tx = new Transaction(doc, "Change Color"))
                {
                    tx.Start();
                ...
                using (var tx = new Transaction(doc, "Set ColorScheme"))
                {
                    tx.Start();
            ...
            {
                var roomCategory = Category.GetCategory(doc, BuiltInCategory.OST_Rooms);
                var listColorScheme = new FilteredElementCollector(doc)
                    .OfCategory(BuiltInCategory.OST_ColorFillSchema)
                    .Cast<ColorFillScheme>()
                    .Where(c => c.CategoryId == roomCategory.Id)
                    .ToList();
                ...
                foreach (ColorFillScheme scheme in listColorScheme) listName.Add(scheme.Name);
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            Log.Logger = new LoggerConfiguration()
                ...
                .CreateLogger();
            Log.Information("start RoutePgm");
