...
namespace Revit.Green3DScan.Routing
{
    [Transaction(TransactionMode.Manual)]
    public class RoutePgmExport : IExternalCommand
    {
        private static Result GetUIView(UIDocument uidoc, SettingsJson set, Transform trans, out double pngWidthInMeter,
            out XYZ bottomLeft)
        {
            pngWidthInMeter = 0;
            bottomLeft = null;
            try
            {
                ...
                {
                    UIView uIView = uidoc.Application.ActiveUIDocument.GetOpenUIViews()
                        .FirstOrDefault(viewUi => viewUi.ViewId == view.Id);

                    if (uIView != null)
                    {
                        var corners = uIView.GetZoomCorners();
                        XYZ min = trans.OfPoint(new XYZ(corners[0].X, corners[0].Y, corners[0].Z)) *
                                  Constants.feet2Meter;
                        XYZ max = trans.OfPoint(new XYZ(corners[1].X, corners[1].Y, corners[1].Z)) *
                                  Constants.feet2Meter;
                        ...
                        var centrum = new XYZ((min.X + max.X) / 2, (min.Y + max.Y) / 2, (min.Z + max.Z) / 2);
                        bottomLeft = centrum - new XYZ(0.5 * set.PgmImageExpansion_Px * set.PgmImageResolution_Meter,
                            0.5 * set.PgmImageExpansion_Px * set.PgmImageResolution_Meter, 0);
                    }
                ...
                {
                    var minExpansion = (int)Math.Round(pngWidthInMeter / set.PgmImageResolution_Meter);
                    TaskDialog.Show("Message",
                        "meters! This value results from the settings and the current zoom. Set at least " +
                        minExpansion + " pixels for this section.");
                    return Result.Failed;
                }

                if (pngWidthInMeter == 0)
                {
        ...
        private static Result ExportToPng(Document doc, string filepath, double modelSizeInMeter, SettingsJson set)
        {
            try
            {
                using var tx = new Transaction(doc, "Export Image");
                tx.Start();
        ...
        private static Result ConvertToPgm(string pathPng, string pathPgm, SettingsJson set)
        {
            try
            {
                byte black = 0;
                ...
                var targetWidth = (int)set.PgmImageExpansion_Px;
                var targetHeight = (int)set.PgmImageExpansion_Px;

                // target image with grey background
                var targetImage = new Mat(targetHeight, targetWidth, MatType.CV_8UC1, new Scalar(205));
                ...
                for (var y = 0; y < originalImage.Rows; y++)
                for (var x = 0; x < originalImage.Cols; x++)
                {
                ...
                imageAfterChange.CopyTo(
                    targetImage[new Rect(offsetX, offsetY, originalImage.Cols, originalImage.Rows)]);
            ...
            static Mat ChancheGreyValue(Mat image, byte oldValue, byte newValue)
            {
                ...
                for (var x = 0; x < image.Cols; x++)
                {
                    ...
                    if (pixelValue == oldValue) image.Set(y, x, newValue);
                }
        ...
        private static Result ConvertToPgmWithouExtension(string pathPng, string pathPgm, SettingsJson set)
        {
            try
            {
                byte black = 0;
        ...
        private static Result WriteYaml(string pathYaml, SettingsJson set, XYZ bottomLeft)
        {
            string fileName = Path.GetFileNameWithoutExtension(pathYaml);
            try
            {
                var wert =
                    $"[{bottomLeft.X.ToString(Sys.InvariantCulture)}, {bottomLeft.Y.ToString(Sys.InvariantCulture)}, 0.000000]";
            ...
            void SerializeToYaml(Dictionary<string, object> data, string pathYaml)
            {
                ...
                using (var writer = new StreamWriter(pathYaml)) serializer.Serialize(writer, data);
            }
        }
        ...
        public class ViewCreation
        {
            public static IEnumerable<ViewFamilyType> FindViewTypes(Document doc, ViewType viewType)
            {
                var ret = new FilteredElementCollector(doc)
                    .WherePasses(new ElementClassFilter(typeof(ViewFamilyType), false)).Cast<ViewFamilyType>();
            ...
            public static ViewPlan NewViewPlan(Level level, ViewType viewType)
            {
                ...
                var view = ViewPlan.Create(level.Document, viewTypeId, level.Id);
                return view;
            }
        }
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (!Directory.Exists(routePath)) Directory.CreateDirectory(routePath);
